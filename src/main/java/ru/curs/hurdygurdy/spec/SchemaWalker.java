...
/**
 * Visits every schema reachable from an OpenAPI document, in document order,
 * handing each one to a visitor together with a JSON-pointer-like path
 * describing where it was found (e.g.
 * {@code #/components/schemas/Sample/properties/opt}).
 *
 * <p>Two things need this walk and must agree on it exactly: {@link
 * StrayNullableCheck}, which reports where a 3.1 document still carries a
 * {@code nullable}, and {@link SchemaNormalizer}, which rewrites 3.1-only
 * spellings into their canonical form. Sharing one traversal is what keeps the
 * warning and the rewrite talking about the same set of schemas — a schema the
 * walk misses is one the generator handles without ever having been checked.
 *
 * <p>A visitor sees a schema <em>before</em> that schema's children are walked,
 * so a visitor that replaces a child (as the normalizer does with a missing
 * {@code items}) can rely on the replacement being walked in turn.
 *
...
public final class SchemaWalker {
    ...
    private final BiConsumer<Schema<?>, String> visitor;

    private SchemaWalker(BiConsumer<Schema<?>, String> visitor) {
        this.visitor = visitor;
    }

    /**
     * Applies {@code visitor} to every schema in {@code openAPI}, each with the
     * path at which it was reached. Does nothing for a null document.
     *
